...
def createCar(count : int, scenicObj):
    ...
    if scenicObj.xPos != 0 or scenicObj.yPos != 0:
        traci.vehicle.moveToXY(scenicObj.name, road, scenicObj.lane,  scenicObj.xPos, scenicObj.yPos, angle = scenicObj.angle, keepRoute = scenicObj.vehPlacement)
...
def iterateScene(scene : tuple):
    count = 0   
    for scenicObj in scene[0].objects:
        if str(type(scenicObj)) == "<class 'scenic.simulators.sumo.model.Car'>":
...
def createParkingLot(scenicObj):
    ...
    xml.SubElement(root,"parkingArea", id=scenicObj.name, lane=scenicObj.lane,startPos=str(scenicObj.startPos), endPos=str(scenicObj.endPos),roadsideCapacity=str(scenicObj.capacity),angle=str(scenicObj.angle),length="5").text
    xml.SubElement(root, "vType", id="example", maneuverAngleTimes="10 3.0 4.0,80 1.6 11.0,110 11.0 2.0,170 8.1 3.0,181 3.0 4.0")
